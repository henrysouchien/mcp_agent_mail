...
install_cli_launcher() {
  # Install the canonical CLI entry point backed by this checkout's virtualenv.
  local launcher_dir="${HOME}/.local/bin"
  local launcher_path="${launcher_dir}/mcp-agent-mail"
  local python_path="${REPO_DIR}/.venv/bin/python"
...
if [[ ! -x "\${python_path}" ]]; then
  printf 'mcp-agent-mail: Python environment not found at %s\\n' "\${python_path}" >&2
  printf 'Re-run scripts/install.sh --dir %q --no-start\\n' "${REPO_DIR}" >&2
  exit 1
fi
export MCP_AGENT_MAIL_PROG_NAME="mcp-agent-mail"
exec "\${python_path}" -m mcp_agent_mail.cli "\$@"
LAUNCHER_EOF
  ...
  for variant in "mcp_agent_mail" "mcpagentmail" "agentmail" "agent-mail"; do
    local variant_path="${launcher_dir}/${variant}"
    if [[ ! -f "${variant_path}" ]]; then
      ln -sf "${launcher_path}" "${variant_path}" 2>/dev/null || true
    fi
  done

  ok "Installed canonical CLI launcher at ${launcher_path}"
  record_summary "CLI launcher: ${launcher_path}"
}
