...
## Product Bus (Operator/Legacy Unless Independently Verified)

Do not assume Product Bus mutations use the core route/audit/idempotency
contract. Treat them as operator/legacy features until each deployed path has
been independently verified for core.
...
## Build Slots (Legacy/Migration Only)

Build-slot mutations open the legacy archive and are unavailable in core mode.
...
- Configures user-scope MCP servers in root `~/.claude.json`
...
## Legacy/Migration Git Archive Layout

This filesystem tree is the optional legacy archive, not core authoritative
message storage.
...
Core stores canonical metadata, messages, recipients, reservations, audit
events, idempotency receipts, and credential records in `DATABASE_URL`.
Attachment bytes live under `BLOB_STORAGE_ROOT` as content-addressed objects;
database blob references and install leases protect lifecycle and garbage
collection. Core does not create or consult the Git archive for message
durability.
...
|----------|---------|-------------|
| `RUNTIME_PROFILE` | deployment-defined | Set `core` for Git-independent runtime |
| `DATABASE_URL` | `sqlite+aiosqlite:///./storage.sqlite3` | Durable core database |
| `BLOB_STORAGE_ROOT` | `~/.mcp_agent_mail/blobs` | Durable content-addressed blob root |
| `CORE_OWNER_TOKEN` | — | Secret owner/control-plane bearer |
| `CREDENTIAL_PEPPERS_JSON` | — | Secret map of base64url credential peppers |
| `CREDENTIAL_CURRENT_PEPPER_KEY_ID` | — | Active key in the pepper map |
| `STORAGE_ROOT` | `~/.mcp_agent_mail_git_mailbox_repo` | Legacy/migration Git archive root |
...
This section describes legacy/migration and optional code-repository identity
features. Core Agent Mail message persistence is Git-independent; user code
repositories may still use Git normally.
