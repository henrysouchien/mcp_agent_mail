...
Liveness proves only that a process is serving. Readiness proves that its
startup and storage checks passed. Also verify the process start time/build
when validating a rollout; an old healthy process has not loaded new code.

For a timed-out mutation, do not immediately repair the database or resend with
a fresh idempotency key. Replay the exact logical request with its original key
or reconcile the resulting receipt/state first. A fresh key can create the
duplicate the idempotency protocol was designed to prevent.

---

## Legacy/Migration Doctor Commands

The current doctor and repair commands import legacy storage and may require
the optional Git dependency. Do not run them against a core deployment. Core
diagnostics use liveness/readiness plus database audit-ledger and blob-store
inspection under the deployment's normal operational controls.
...
- Orphaned records
- FTS index sync
- Legacy Git archive consistency (legacy/migration projects only)

Core projects use the database audit ledger and blob store as authoritative
state. Archive repair/restore commands below are legacy compatibility tools,
not the core message recovery path.
...
| "database is locked" | Concurrent access issue | Preserve the original request and idempotency key; back off and reconcile its receipt/state. Use a coordinated service-manager restart only when readiness/log evidence requires it. |
