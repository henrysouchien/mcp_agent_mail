...
- Ordinary development work happens on the user's existing `main` checkout. Do not create, delete, rename, or otherwise manage branches or worktrees as part of routine implementation.
- An agent may switch the existing checkout when the user explicitly requests checkout recovery, fork synchronization, or branch reconciliation. Before switching, inspect status/remotes and preserve every tracked and untracked change with a named, reversible stash or a user-approved commit.
- Repository-maintenance switching does not authorize destructive commands, history rewriting, force pushes, branch deletion, or worktree creation. Those require separate, explicit user approval and remain subject to the irreversible-action rules above.
- Finish reconciliation on `main` before making substantive implementation changes, and report any preserved stash or unresolved branch state to the user.
